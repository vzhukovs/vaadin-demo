# Vaadin Demo Project with Machine Exec Support
...
`java -jar target/vaadindemoprojectwithmachineexecsupport-1.0-SNAPSHOT.jar`
...
```
docker build . -t vaadindemoprojectwithmachineexecsupport:latest
```

Once the Docker image is correctly built, you can test it locally using

```
docker run -p 8080:8080 vaadindemoprojectwithmachineexecsupport:latest
```
...
If the pods say `Container image "vaadindemoprojectwithmachineexecsupport:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
